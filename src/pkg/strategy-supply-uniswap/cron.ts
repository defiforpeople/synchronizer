...
import { CronEvent, SupplyUniswapEvent, SupplyUniswapStrategy } from "./type";
...
export class Cron {
  ...
  private storage: ISupplyUniswapStorage;
    ...
    this.interval = setInterval(async () => {
      ...
      try {
        ...
        if (deposits.length > 0) {
          console.log("Supply Uniswap: Inserting deposits in db", deposits);
          await this.storage.insertEvents(deposits);
          this.latestDeposit.block = deposits[deposits.length - 1].block;
        }
      } catch (err: any) {
        console.warn(`Supply Uniswap: Could't get or insert deposit event, err=${err.message}`);
      }
      ...
      try {
        ...
        if (withdraws.length > 0) {
          console.log("Supply Uniswap: Inserting withdraw in db", withdraws);
          await this.storage.insertEvents(withdraws);
          this.latestWithdraw.block = withdraws[withdraws.length - 1].block;
        }
      } catch (err: any) {
        console.warn(`Supply Uniswap: Could't get or insert withdraw event, err=${err.message}`);
      }
    }, this.intervalMs);
  }

  private async getNewEvents(type: EventType, latestBlock: number): Promise<SupplyUniswapEvent[]> {
